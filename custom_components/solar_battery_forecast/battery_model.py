...
class Action:
    ...
    def __repr__(self) -> str:
        return f"Action(ActionType.{self.action_type.name}, {self.min_soc}, {self.max_soc})"
...
class BatteryModel:
    ...
    def shotgun_hillclimb(self, segments: list[TimeSegment], initial_battery: float) -> list[Action]:
            ...
            while True:
                ...
                for slot in slots:
                    ...
                    if actions[slot] is None:
                        actions[slot] = Action(
                            ActionType.SELF_USE, MIN_SOC_PERMITTED_PERCENT / 100, 1.0
        ...
        # Do a final pass. This time, try and simplify: if changing a slot to a "lower" action doesn't hurt the score,
        # do it
        ...
        for slot in range(len(actions)):
            ...
            if not copied_another_action:
                for action_type in action_type_set:
                    if int(action_type) >= int(best_actions_ever[slot].action_type):
                        break
                    prev_action_type = best_actions_ever[slot].action_type
                    best_actions_ever[slot].action_type = action_type
                    new_result = self.run(segments, best_actions_ever, initial_battery)
                    # If the old result was better, go back to it and continue. Otherwise go for the new result
                    if not is_better(best_result_ever, new_result):
                        break
                    # assert(new_score < best_score_ever)
                    best_actions_ever[slot].action_type = prev_action_type
        ...
        ends_of_charge_periods = [
            ...
            and (i == len(actions) - 1 or best_actions_ever[i + 1].action_type != ActionType.CHARGE)
        ]
        ...
        for slot in range(len(actions)):
            ...
            max_soc_percents: Iterable[int]
            if best_actions_ever[slot].action_type == ActionType.CHARGE and slot > 0:
                max_soc_percents = range(
                    100, round(prev_max_soc * 100), -SOC_STEP_PERCENT
                )  # [round(best_actions_ever[slot - 1].max_soc * 100)]
            else:
                #     # Try 1.0 first, then the min soc, then everything in between
                max_soc_percents = [
                    100,
                    round(best_actions_ever[slot].min_soc * 100),
                    *list(range(100 - SOC_STEP_PERCENT, round(prev_max_soc * 100), -SOC_STEP_PERCENT)),
                ]
